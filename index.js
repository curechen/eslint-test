...
// } catch (err) {
//   console.error('Error reading or updating ESLint configuration:', err);
// }

// function addDiffPluginToExtends(config) {
//   const configObject = YAML.load(config); // 使用 YAML.load 解析 YAML 数据
  
//   if (Array.isArray(configObject.extends)) {
//     // 检查是否已经存在该扩展
//     if (!configObject.extends.includes('plugin:diff/diff')) {
//       configObject.extends.push('plugin:diff/diff');
//     }
//   } else {
//     // 如果没有 extends 字段，创建一个包含该扩展的 extends 字段
//     configObject.extends = ['plugin:diff/diff'];
//   }

//   return YAML.dump(configObject); // 使用 YAML.dump 将 JavaScript 对象转换为 YAML 数据
// }
...
function addDiffPluginToExtends(config) {
    ...
    let extendsIndex = lines.findIndex((line) => line.includes('extends: ['));
    ...
    if (extendsIndex !== -1) {
        ...
        extendsIndex = lines.findIndex(line => line.includes('module.exports'));
        if (extendsIndex !== -1) {
            lines.splice(extendsIndex + 1, 0, '  extends: [\'standard\', \'plugin:diff/diff\'],');
        }
    }
